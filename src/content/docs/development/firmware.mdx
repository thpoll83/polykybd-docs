...
The firmware runs the RP2040 at its **125 MHz** default. Raspberry Pi has since certified the chip for **200 MHz**, and you can build for that with one flag:

```sh
qmk compile -kb polykybd/split72 -km default -e POLYKYBD_SYS_CLK=200
```

This is a supported operating point, not an overclock — but it is only certified with the core voltage raised from 1.10 V to **1.15 V**, so the flag does both: it selects the 200 MHz PLL setting *and* raises the regulator before the clock comes up. There is nothing else to configure. Leave the flag off and you get the unchanged 125 MHz build.
...
To confirm what a keyboard is actually running, watch the boot banner on the [firmware console](/development/test-rig/) — it reports the clock and the regulator setting read back from the hardware:
