...
<Aside>
`polyctl` controls an existing daemon — it does not start one. Launch the daemon first with `python -m polyhost` (GUI + daemon) or `python -m polyhost --headless` (no GUI). See [Daemon & Client Model](/software/architecture/).
</Aside>
