...
`Ctrl`+`S` in the middle of one — and the keyboard plays those back happily, but
**nothing writes one yet**: the editor's field and `polyctl macro set --text` both
produce plain characters. A macro that already contains chords or delays is shown as
its steps and left alone rather than flattened into text.
</Aside>
