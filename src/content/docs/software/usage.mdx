...
| **Idle Display** | What the keycaps do while you're away, and — under **Idle After** — how long they wait first. See [Idle & Burn-in Protection](/using/idle/). |
...
| **Help & About** | About, the log viewer, **Open config folder**, and — when something goes wrong — **Report a Problem…** and **Collect logs…** (see [Reporting a Problem](/software/reporting-problems/)). |
...
<Aside type="note">
  This is the menu on the machine the keyboard is plugged into. PolyKybdHost running as
  a [forwarder](/using/multi-machine/#the-forwarders-tray-menu) on another computer has
  its own, shorter menu of the same shape — and a different tray icon, so the two are
  distinguishable when both run on one desktop.
</Aside>
...
The app also sends an anonymous once-a-day usage report, which you are told about on first
run and can switch off under **Settings → Telemetry** — see
[Usage Data & Privacy](/software/telemetry/) for exactly what it contains.
...
To send those logs to someone — or to file a bug with them attached — use **Help & About
→ Report a Problem…** or **Collect logs…** rather than hunting for the files. Both are
described in [Reporting a Problem](/software/reporting-problems/), along with the
`polyctl logs` equivalents that work even when the app will not start.
