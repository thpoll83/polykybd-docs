...
The step editor's **View** switch has a second setting: **Script**, the same macro as a
line of text.
...
Switching between **Table** and **Script** converts the macro rather than replacing it,
so use whichever suits the moment — build a chord by recording it, then switch to Script
to change one keycode. If the script does not parse, the switch is refused and your text
is left alone with the problem named underneath.
