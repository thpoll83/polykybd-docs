...
same position, so the reach is the same muscle memory whichever you own. They **wrap**, so either
arrow gets you round a two-page letter.
...
<Aside type="note">
What gets stored is the variation itself, not its position on the row, so a pick made on page 2 is
the same pick on either model — your accents follow you from a split72 to a split42.
</Aside>
...
### Languages the full set covers

Because the picker now carries every precomposed accented letter, a number of languages that were
previously only partly served are complete:

| Language | What it needs |
|---|---|
| Welsh | `ẁ ẃ ẅ ỳ` — the accented `w` and `y` |
