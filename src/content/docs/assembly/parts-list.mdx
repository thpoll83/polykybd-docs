...
<Aside type="caution">
Each half has a **single** expansion slot, shared by the rotary encoder, a pointing device
(trackball/trackpad) and the light sensor — only **one** of the three fits per half. Fitting the
light sensor therefore replaces the encoder/pointing device on that half. You can still put a
different one on the other half (e.g. the sensor on the left and a trackpad on the right). Decide
which one you want per side before you start.
</Aside>
...
| Option | Part | Notes |
|---|---|---|
| Light + proximity sensor | Pimoroni LTR-559 breakout | Plus a 5-pin 2.54mm pin header. Solders to the labelled `3-6V / SDA / SCL / INT / GND` footprint on the Split72 PCB (shares the Cirque I2C bus). |
