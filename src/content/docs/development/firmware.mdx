...
<Aside type="note">
Signing is rolled out in **warn-only mode** today: the keyboard verifies the signature and logs the result (`OK` / `INVALID` / `UNSIGNED`) but **never blocks the update**. Flashing keeps working with unsigned images and with the committed placeholder key, so there is **no brick risk** while keys and signed releases are being provisioned.
</Aside>
...
To make the keyboard **reject** unsigned or invalid images (instead of just warning), define `FW_REQUIRE_SIGNATURE` at build time — add one line to `keyboards/polykybd/rules.mk`:

```make
OPT_DEFS += -DFW_REQUIRE_SIGNATURE
```

Only do this once a real key is provisioned and your releases are signed — otherwise the firmware will refuse to flash any image. Full key-generation and rotation details are in `keyboards/polykybd/tools/SIGNING.md`.
