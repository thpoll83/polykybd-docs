...
- **On the keyboard** — two keys on the settings layer, one per direction. They carry the
  standard smaller/bigger symbols rather than any text, so they read the same in every
  language, and they stop at Small and Large instead of wrapping around.

  ![The two settings-layer keycaps: a big A beside a small one for smaller, and a small A beside a big one for bigger](../../../assets/using/legend-size-keys.png)
