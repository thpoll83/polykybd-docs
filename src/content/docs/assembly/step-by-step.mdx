...
1. **Solder the optional light + proximity sensor (highly recommended — do this first!)**
   ...
   **before** the plate goes on — adding it later means an almost complete disassembly. It is
   ...
   <Aside type="note">
   Each half has only **two** expansion slots, shared between this sensor, the rotary encoder
   (step 2) and a pointing device (steps 3–4, 7). You can fit **at most two** of the three on the
   same half — so if you add the sensor, drop either the encoder or the pointing device on that half.
   </Aside>
