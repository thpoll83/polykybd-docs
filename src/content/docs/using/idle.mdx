...
<Aside type="tip">
The keyboard key **skips IDDQD** as it cycles. That style is the DOOM easter egg and has its
own way in, so a key that cycled into it would hand it to anyone who pressed twice. A keyboard
already set to it cycles back out normally, and the tray menu and `polyctl` can still select it
directly.
</Aside>
