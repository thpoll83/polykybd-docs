...
- **v17** — **AI_STATE** (`0x28`): the [AI key](/using/ai-key/) wears an AI agent's status — the LED under it and the word on its keycap — and pressing it asks the host to bring the agent's window forward. The status is pushed by this command; the key PRESS travels the other way on the firmware console, because a keycode the firmware handles itself produces no HID traffic of its own.
