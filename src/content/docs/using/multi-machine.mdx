...
## The forwarder's tray menu

The forwarder puts its own icon in the remote machine's tray. Its menu follows the same
shape as [the keyboard machine's](/software/usage/#system-tray-menu), minus everything
that needs a keyboard attached:

![The PolyKybdHost forwarder tray menu: a status line reading "Forwarding to 192.168.1.100", then Pause, Check for host update, Settings, Help & About and Quit](../../../assets/using/tray-menu-forwarder.png)
...
| **Settings** | The handful of settings that mean something here: the light/dark theme and the [browser-URL](/using/website-detection/) options. |
...
<Aside type="tip">
  **The icon tells the two apps apart.** Both marks are a grid of keycaps whose unlit
  keys spell a letter: **P** on the machine with the keyboard, **F** on a forwarder. If
  you forward to your own desktop you will see both in the same tray, so it is worth
  knowing which is which. The forwarder's mark also goes **grey when its reports are not
  arriving**, and back to colour when they are.
</Aside>

### Pausing

**Pause** stops this machine's window titles leaving it — useful when you are doing
something on the remote machine that you would rather the other computer knew nothing
about. Nothing is sent while it is paused, and the status line says so. Resuming sends
the current window straight away, so the keycaps catch up without waiting for you to
switch application.

<Aside type="note">
  Pause is per-machine and is not remembered across restarts. It also only affects
  *this* forwarder — the keyboard keeps following whatever the keyboard machine, and
  any other forwarder, reports.
</Aside>

## Checking that it works

The forwarder's status line answers this before you open anything: it reads
**Forwarding to &lt;address&gt;** when reports are landing and **Cannot reach
&lt;address&gt;** when they are not, and the tray mark greys out to match.
...
Collecting the evidence is one click on each side: the forwarder's **Help & About**
submenu has the same **Report a Problem…** and **Collect logs…** entries as the keyboard
machine's, and a forwarder report says so on its first line. Because the two halves run on different
