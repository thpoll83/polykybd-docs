...
A firmware you compiled yourself is unsigned, so it will be refused over HID. Two ways through:
...
2. **Or arm the override on the keyboard**: press the *allow unsigned firmware* key (settings layer, beside the Eden key), then start the flash within **2 minutes**. It is single-use and cleared by a reboot.
</Steps>

<Aside type="note">
The override is a **keypress**, not a checkbox in the host app — deliberately. Signing protects against anything that can talk to the keyboard over HID, and a confirmation sent over that same channel could be sent by an attacker just as easily. A physical keypress cannot be produced remotely.
</Aside>
