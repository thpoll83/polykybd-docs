...
All standard QMK features are available. The fork is currently based on **QMK
0.33.13**, so where upstream behaviour has changed between releases, the 0.33.x
documentation is the one that matches your build.
...
<Aside type="caution">
On a shallow clone (the default in some cloud dev containers) `git merge-base`
returns an **empty string** instead of an error, and the command above silently
reports the wrong thing. Run `git rev-parse --is-shallow-repository` first, and
`git fetch origin --unshallow` if it says `true`.
</Aside>
