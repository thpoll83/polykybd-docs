...
| `ai status\|state [<state>]\|target [<pattern>]` | Drive the [AI key](/using/ai-key/) — `state` sets what it shows (`off`/`idle`/`working`/`attention`, plus aliases like `busy`, `done` and `waiting`), `target` sets which window a press raises (part of the title, or `/a regex/`), `status` prints both plus every window matching right now |
