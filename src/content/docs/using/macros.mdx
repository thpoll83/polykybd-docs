...
`--text` writes plain characters only; **chords and pauses are written in the step
editor**, not from the command line. `polyctl macro get` will show them, so a macro
built in the editor is still readable from a script.
