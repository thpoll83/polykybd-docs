...
<Aside type="caution">
Signature enforcement is **on**. An image without a valid signature is **refused** at the end of a firmware update over HID. Released firmware is signed automatically, so normal updates are unaffected — but a build you compiled yourself needs either a signature or a physical confirmation on the keyboard, described below. Flashing over **BOOTSEL/UF2 is never affected**, so there is no way to lock yourself out.
</Aside>
...
With it defined, an unsigned or badly-signed image is rejected instead of merely logged. Only enable this on a fork once your own key is provisioned **and** your releases are signed — otherwise the firmware refuses every image you build.
...
2. **Or confirm it on the keyboard.** Just flash. At the end of the transfer the keyboard notices the image is unsigned and **turns its keycaps into a dialog**: every key goes dark except one on each half — a big **A / ACCEPT** on the left home-row index key (`D`) and a big **R / REJECT** on the right one (`J`). Press **A** to let this image through. The host shows *"Confirm on the KEYBOARD…"* and waits for you.
