...
| **SET_UNICODE_MODE** | `0x14` (20) | Select the OS unicode-input mode. **v17+** adds a *volatile* flag: the mode applies in RAM only and the keyboard's stored choice is left alone, which is how the host applies a reading it is not yet sure of. |
...
| **AI_STATE** | `0x28` (40) | **v18+.** Get/set what the [AI key](/using/ai-key/) shows — `0` off, `1` idle, `2` busy, `3` needs you; `0xFF` queries. RAM only: the state describes something running on the host, so it means nothing after a reboot and the host re-sends it on reconnect. The range is closed — an unknown value is refused, because every value names both a colour to light and a word to draw. |
...
- **v17** — a *volatile* flag on **SET_UNICODE_MODE** (`0x14`): the mode is applied in RAM and the keyboard's stored choice is left untouched. At Windows logon the host cannot yet tell "WinCompose is not installed" from "WinCompose has not started yet", so it applies its first reading volatile and re-asserts it for keeps once it can tell the two apart — otherwise every logon on a WinCompose machine would write the wrong mode and then write it back.
- **v18** — **AI_STATE** (`0x28`): the [AI key](/using/ai-key/) wears an AI agent's status — the LED under it and the word on its keycap — and pressing it asks the host to bring the agent's window forward. The status is pushed by this command; the key PRESS travels the other way on the firmware console, because a keycode the firmware handles itself produces no HID traffic of its own.
