...
built into its firmware before applying an update. Today this is informational (the result is logged
but an unsigned image still flashes), so nothing changes for you. If you build and sign your own
firmware — or want to require signatures — see [Firmware signing](/development/firmware/#firmware-signing).
</Aside>
