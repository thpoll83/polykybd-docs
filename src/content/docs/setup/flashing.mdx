...
To update from a phone instead of a computer, see
[Flash from an Android Phone](/setup/flashing-android/). It uses the same HID path.
...
<Aside type="caution">
Official firmware releases are **signed**, and the keyboard verifies the signature against a key
built into its firmware before applying an update.

The **release-update flow in the tray app handles this for you** — it downloads the release's
`.bin.sig` alongside the `.bin`, and stops with an explanation rather than flashing if the signature
can't be fetched. If you download a release **by hand** instead, take the `.bin.sig` too and **put it
in the same folder as the `.bin`**: the host looks for it right there, and without it the keyboard
treats the image as unsigned.

An unsigned image is not rejected, but it is not applied silently either: the keyboard blanks its
keycaps except a big **A / ACCEPT** on the left half and **R / REJECT** on the right, states the
question on the status OLED, and waits up to a minute for you to press one. That is expected for a
firmware you built yourself. If the host instead reports that the signature *does not match*, the
`.bin` and `.sig` are from different builds or one of them is damaged — re-download both.

If you build and sign your own firmware — or want to require signatures — see
[Firmware signing](/development/firmware/#firmware-signing).
</Aside>
