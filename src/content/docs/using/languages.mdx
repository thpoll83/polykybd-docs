...
switch. 19 letters carry variations, up to 10 each; `a`, `e`, `i`, `o` and `u` have the most.
...
3. Tap **Ctrl** [ Á»Æ ]. The Ctrl keycap turns **inverted** — dark legend on a lit key — to show
   the picker is armed, and the number row fills with every variation available for that letter.
4. Press the digit under the accent you want.
...
[keymap editor](/using/keymap-editor/).
</Aside>
