...
built into its firmware before applying an update. **Download the release's `.bin.sig` file into the
same folder as the `.bin`** — the host sends it during the flash, and without it the keyboard treats
the image as unsigned.
