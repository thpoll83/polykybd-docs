...
keypress, so the very next key you press fires its macro. To stay on the layer instead — for
...
![The twelve macro keycaps on the Utility layer. Without a modifier they read Macro 0 to Macro 11, each with the Mayan numeral for its own number above the caption. With Shift held the first four become Macro 12 to Macro 15 and the remaining eight are blank.](../../../assets/using/macro-row.png)

<Aside type="caution" title="F13–F24 no longer have a home">
That row used to be **F13** through **F24**. If you use them, put them back from the Keymap
Editor — they are still perfectly ordinary keycodes, they just are not on the default keymap any
more. Almost nothing sends them, and this is the only row on the board where sixteen macros are
reachable without the host app at all.
</Aside>
...
letters they are — and on which face the keyboard picks. A label that fits gets the **larger of
two faces**, which is why `push` and `work mail` are drawn half again as big as a long one; past
that width the keyboard drops to the small face and keeps every character rather than trimming
one to stay large. About **nine characters** at the large face, **twelve** at the small one.
...
The label sits along the bottom edge, in the largest of two faces that fits it whole — short
labels are noticeably bigger than long ones, and a label too wide for the larger face keeps all
its characters at the smaller one rather than losing any to stay large. A macro
...
A macro you have never touched is not blank. Each of the sixteen starts with the **Mayan numeral
for its own number** over the caption `Macro 0`, `Macro 1`, and so on — a shell for zero, then
dots and bars up to `Macro 15`.

They are numerals rather than pictures on purpose: a slot nobody has written yet has no purpose,
so a gear or an envelope would be a wrong label rather than a neutral one. The numeral says the
same thing the caption does, and unlike a row of identical keys it tells you at a glance which
one your finger is on.

Anything you set replaces it and is kept — the stock look only ever fills a slot that has no
macro **and** no look of its own. Clearing a macro hands its keycap the numeral back.

<Aside type="note" title="They come from the font pack">
The numerals live in the keyboard's `symbol` font pack, which the host app flashes on connect. On
