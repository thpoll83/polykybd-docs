...
| `macro list\|get <id>\|set <id> …\|clear <id>` | Read and write the keyboard's [macros](/using/macros/) — `set` takes `--text`, `--label`, `--style` and `--icon`; `list` also reports how much of the shared store is in use |
