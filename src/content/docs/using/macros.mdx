...
![The twelve macro keycaps on the Utility layer. Each is captioned Macro, with the Mayan numeral for its own number above it. With Shift held the first four show the numerals for twelve to fifteen and the remaining eight are blank.](../../../assets/using/macro-row.png)
...
so a gear or an envelope would be a wrong label rather than a neutral one. The numeral carries
the number so the caption does not have to, and unlike a row of identical keys it tells you at a
glance which one your finger is on.
